...
from dice_10001.scoring import DiceCount, filter_outcomes, get_outcomes, is_bust
...
def find_unique_outcomes():
    ...
    for dice_count in range(1, 7):
        ...
        for roll, weight in generate_rolls(dice_count):
            for outcome in filter_outcomes(get_outcomes(roll)):
...
def _naive_points_strategy():
    ...
    for dice_count in range(1, 7):
        ...
        for roll, weight in generate_rolls(dice_count):
            ...
            outcome = max(filter_outcomes(get_outcomes(roll)), key=lambda x: x.points)
